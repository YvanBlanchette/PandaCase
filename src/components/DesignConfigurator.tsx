...
import { COLORS, FINISH, MATERIALS, MODELS } from "@/validators/option-validator";
		...
		<div className="relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20">
			<div className="relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
				<div className="relative w-60 bg-opacity-50 pointer-events-none aspect-[896/1831]">
					...
					<div className="absolute z-40 inset-0 left-[3px] top-px right-[3px] bottom-px rounded-[32px] shadow-[0_0_0_99999px_rgba(229,231,235,0.7)] " />
					<div className={cn("absolute inset-0 left-[3px] top-px right-[3px] bottom-px rounded-[32px]", `bg-${options.color.tw}`)} />
				</div>
				<Rnd
					default={{ x: 150, y: 205, height: imageDimensions.height / 2, width: imageDimensions.width / 2 }}
					lockAspectRatio
				...
				>
					<div className="relative w-full h-full">
						<NextImage alt="custom image" src={imageUrl} className="pointer-event-none" fill />
					</div>
				</Rnd>
			</div>
			<div className="h-[37.5rem] w-full col-span-full lg:col-span-1 flex flex-col bg-white">
				<ScrollArea className="relative flex-1 overflow-auto">
					...
					<div className="px-8 pb-12 pt-8">
						<h2 className="text-3xl text-center lg:text-left font-bold tracking-tight">Customize your case</h2>
									...
									<DropdownMenu>
										...
										<DropdownMenuContent>
											{MODELS.options.map((model) => (
												<DropdownMenuItem
													key={model.label}
													className={cn("flex text-sm gap-1 items-center p-1.5 cursor-default hover:bg-zinc-100 w-full", {
												...
												>
													<Check className={cn("mr-2 h-4 w-4 opacity-0 text-green-600", { "opacity-100": model.label === options.model.label })} />
													{model.label}
												</DropdownMenuItem>
											))}
										</DropdownMenuContent>
				...
				<div className="w-full px-8 h-16 bg-white">
					...
					<div className="w-full h-full flex justify-end items-center">
						<div className="w-full flex gap-6 items-center justify-between">
							<p className="font-medium whitespace-nowrap">{formatPrice((BASE_PRICE + options.materials.price + options.finish.price) / 100)}</p>
